...
import { QuizType, ProgressionMode, ScoringStrategy, DecayStartSource } from '@/lib/types';
...
export default function EditQuizPage() {
  ...
  useEffect(() => {
    if (quiz) {
      ...
      setDecayStartSource(quiz.decay_start_source || 'question_open');
      setDecayMinPoints(quiz.decay_min_points ?? 1);
    }
  }, [quiz]);
      ...
      <form onSubmit={handleUpdate} className="space-y-8">
        ...
        <div className="p-6 sm:p-8 rounded-3xl bg-white border border-[#ebdcd1] shadow-sm space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="block text-xs font-bold text-slate-700">Time Limit per Question (seconds)</label>
                <span className="text-[10px] font-bold text-slate-500">
                  {timeLimitSec === 0 ? '∞ Unlimited (No constraint)' : `${timeLimitSec}s`}
                </span>
              </div>
              <input
                type="number"
